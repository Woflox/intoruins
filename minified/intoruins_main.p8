...
if modeis"play"or
modeis"ui"then barx=2drawbar("HP",player.hp,player.maxhp,2,8)for k,v in pairs(player.statuses)do drawbar(unpack(v))end elseif modeis"aim"then call"print(\f6⬅️\+fd⬆️\+8m⬇️\+fd➡️:aIM     ❎:fIRE,18,118"end if modeis"ui"then
...
entropy-=0.013if tl then tl.visitadjrnd(function(ntl)if not ntl.tileflag"4"then
if inbounds(ntl.pos)and
rndp(entropy)then gentile(tl.typ,ntl)if ntl.tileflag"4"then if rndp(0.005+depth*0.001)then genroom(rnd(inboundposes))end gencave(ntl)end end end end)end end function gentile(typ,_ENV)local y=ceil(rnd"15")
if(manmade)y+=16
...
postgenned then if not genned then gentile(tl.typ,_ENV)end postgen(_ENV,tl.tileflag"4"and tl or prevtl)end end)end function postproc()function connectareas(permissive)for i=1,20do calcdist("pdist",gentl)local unreach,numtls,bestdist={},0,100alltiles(function(_ENV)
if navigable()and
...
adjtl[5].tileflag"4"then gentile(tcavewall,_ENV)end end end)connectareas(true)local numholes,furthestd=0,0function checkspawn(_ENV,_typ,mindist,allowent)
if navigable()and
