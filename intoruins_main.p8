					...
					then
					 if rndp(crumble) and
					    ...
					    (xwall and ywall) then				   
							gentile(txwall,tl)
							genned=ent--still want eg. grass to spread here
						else
							set(
...
function checkspawn(_ENV,_typ,mindist,allowent)
	if navigable() and
		...
		(allowent or not ent)
	then	
		return not _typ or create(mapping[_typ] or _typ,pos)
	end
end
...
function postproc()
 ...
 ca()
	...
	do
		local _ENV=player
		setpos(genpos)
		call"calcdist(pdist)calclight()updateenv("
	...
	for n=1,7-depth\5.99 do
		checkspawn(rndtl(),mget(64+rndint"59",24),-3)
	end
	--rubberbanding items
	...
	--printh"\n\n====rubberbanding====\n"
	call"rband(life,248)rband(slofall,249)rband(empower,250)rband(identify,251)rband(light,252)rband(wpn,132|133|134|135|316|317|318|319,0)rband(wearable,308|309|310|312|313|314,0)calcvis()calclight(,t,t,t"
end
