...
if(flippable or ai)
and rndp"0.5"then xface*=-1end if ai and rndp"0.5"then yface*=-1end checkidle()if behavis"sleep"then setanim"sleep"end return _ENV end function pickup(_ENV)if _ENV then if not beenfound then beenfound=true _g.itemsfound+=1end if#inventory<10then sfx"25"addtoinventory()if victory then player.setanim"victory"player.yface,player.statuses,tl.fire,light=-1,{},0call"setmode(victory)calcvis()calclight("end setpos()log("+"..getname())else log"INVENTORY FULL"end end end function turn(btnid,i)if getbtn(btnid)then player.diri=(player.diri+i)%6+1player.setanim"turn"end end function updateplayer()if player.taketurn()then player.tickstatuses()call"calcdist(pdist)calcvis("updateturn=function()calclight()for i,_ENV in inext,ents do if ai then taketurn()end if not isplayer then tickstatuses()end end updateturn=function()for i,_ENV in inext,ents do
